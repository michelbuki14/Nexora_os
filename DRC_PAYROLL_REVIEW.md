...
**Status**: ✅ Current — requires annual validation with DGI (Direction Générale des Impôts)
...
```rust
// Configuration for dual-currency handling
struct DualCurrencyConfig {
    base_currency: CurrencyCode,     // Always CDF for DRC
    reference_currency: CurrencyCode, // USD for expat contracts
    rate_source: ExchangeRateSource,  // BCC official rate
    rate_date: RateDate,             // Payment date vs contract date
    smig_floor_cdf: Money,           // SMIG floor in CDF
}
```
...
**Public Holidays Requiring Double Pay (100% premium)**:
- Jan 1 (New Year), Jan 4 (Martyrs' Day), May 1 (Labor Day), Jun 30 (Independence), Aug 1 (Parents' Day), Dec 25 (Christmas), plus religious holidays (Eid, Easter Monday)
...
The following scenarios **must** be encoded as golden file tests in `test/fixtures/golden/drc_payroll/`:
...
| `DRC-002` | Employee with spouse + 2 children | All exemptions applied correctly |
| `DRC-003` | High earner exceeding CNSS ceiling | CNSS capped, IPR on full gross |
| `DRC-004` | Mid-year hire (July 15) | Prorated IPR, CNSS, leave accrual |
| `DRC-005` | Termination mid-period (Oct 20) | Severance, notice pay, pro-rata leave |
| `DRC-006` | Expatriate USD contract with CDF floor | BCC rate on payment date, SMIG floor enforced |
| `DRC-007` | Maternity leave (14 weeks) | CNSS maternity benefit, employer top-up |
| `DRC-008` | Overtime + night shift + Sunday | 130%/160%/200% premiums, CNSS on total |
| `DRC-009` | Piece-rate worker below SMIG | SMIG top-up triggered |
| `DRC-010` | Annual IPR reconciliation (DAS) | Monthly totals = annual declaration |
