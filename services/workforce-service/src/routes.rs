...
use aos_common::{
    ...
    tenant_context::{add_rls_middleware, RlsState},
};
...
pub fn workforce_router(rls_state: RlsState, auth_state: AuthState) -> Router<AppState> {
    Router::new()
        ...
        .route("/employees/:employee_ulid/status", patch(update_employee_status))
        .route("/employees/:employee_ulid/employment", patch(update_employment))
        .route("/employees/:employee_ulid/compensation", post(create_compensation))
        .route("/employees/:employee_ulid/compensation", get(get_compensation))
        .route("/employees/:employee_ulid/documents", post(create_document))
        .route("/employees/:employee_ulid/documents", get(list_documents))
        .route("/employees/:employee_ulid/documents/:doc_ulid/url", get(get_document_url))
        // Auth is outermost (added last), RLS is next, then the business routes.
        .layer(add_rls_middleware(rls_state))
