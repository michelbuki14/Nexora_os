...
pub async fn create_legal_entity(
    ...
    let country_id: Option<uuid::Uuid> = if let Some(ref cu) = req.country_ulid {
        Some(resolve_ulid(&mut conn, "countries", cu).await?)
    } else {
        None
    };
...
pub async fn list_legal_entities(
    ...
    let rows = sqlx::query_as::<_, (String, String, Option<String>, String, chrono::DateTime<chrono::Utc>)>(
    ...
    let total: i64 = sqlx::query_scalar(
        "SELECT COUNT(*) FROM wf_legal_entities WHERE deleted_at IS NULL",
    )
    .fetch_one(&mut *conn)
    .await?;
    ...
    let items: Vec<LegalEntityResponse> = rows
        .into_iter()
        .map(|(ulid, name, registration_number, status, created_at)| LegalEntityResponse {
            ulid, name, registration_number, status, created_at,
        })
        .collect();

    Ok(Json(PaginatedResponse { items, total, page: q.page, page_size: q.page_size }))
}
...
pub async fn create_location(
    ...
    let tenant_id = resolve_tenant_id(&mut conn, &auth).await?;
    let org_id = resolve_org_id(&mut conn, &auth).await?;
    let entity_id = resolve_ulid(&mut conn, "wf_legal_entities", &req.legal_entity_ulid).await?;
    let tz = req.timezone.unwrap_or_else(|| "Africa/Lubumbashi".to_string());
    let address = req.address.unwrap_or_else(|| serde_json::json!({}));
    ...
    emit_workforce_event(&mut conn, &auth, tenant_id, org_id, "location", &ulid,
        "location.created", serde_json::json!({"name": req.name})).await?;

    Ok((StatusCode::CREATED, Json(LocationResponse {
        ulid, name: req.name, code: req.code, timezone: tz,
...
pub async fn list_locations(
    ...
    let rows = sqlx::query_as::<_, (String, String, Option<String>, String, bool, chrono::DateTime<chrono::Utc>)>(
        "SELECT ulid, name, code, timezone, is_active, created_at
         FROM wf_locations WHERE deleted_at IS NULL
         ORDER BY created_at DESC LIMIT $1 OFFSET $2",
    )
    .bind(q.page_size).bind(q.offset())
    .fetch_all(&mut *conn).await?;

    let total: i64 = sqlx::query_scalar(
        "SELECT COUNT(*) FROM wf_locations WHERE deleted_at IS NULL",
    ).fetch_one(&mut *conn).await?;

    let items: Vec<LocationResponse> = rows.into_iter()
        .map(|(ulid, name, code, timezone, is_active, created_at)| LocationResponse {
            ulid, name, code, timezone, is_active, created_at,
        }).collect();
...
pub async fn create_department(
    ...
    let parent_id: Option<uuid::Uuid> = if let Some(ref pu) = req.parent_department_ulid {
        Some(resolve_ulid(&mut conn, "wf_departments", pu).await?)
    } else {
        None
    };
    let location_id: Option<uuid::Uuid> = if let Some(ref lu) = req.location_ulid {
        Some(resolve_ulid(&mut conn, "wf_locations", lu).await?)
    } else {
        None
    };
    ...
    .bind(&ulid).bind(tenant_id).bind(org_id)
    .bind(&req.name).bind(&req.code).bind(parent_id).bind(location_id)
    .execute(&mut *conn).await?;

    emit_workforce_event(&mut conn, &auth, tenant_id, org_id, "department", &ulid,
        "department.created", serde_json::json!({"name": req.name})).await?;

    Ok((StatusCode::CREATED, Json(DepartmentResponse {
        ulid, name: req.name, code: req.code, is_active: true, created_at: chrono::Utc::now(),
    })))
}
...
pub async fn list_departments(
    ...
    let rows = sqlx::query_as::<_, (String, String, Option<String>, bool, chrono::DateTime<chrono::Utc>)>(
    ...
    let total: i64 = sqlx::query_scalar(
        "SELECT COUNT(*) FROM wf_departments WHERE deleted_at IS NULL",
    ).fetch_one(&mut *conn).await?;

    let items: Vec<DepartmentResponse> = rows.into_iter()
        .map(|(ulid, name, code, is_active, created_at)| DepartmentResponse {
...
pub async fn create_team(
    ...
    let tenant_id = resolve_tenant_id(&mut conn, &auth).await?;
    let org_id = resolve_org_id(&mut conn, &auth).await?;
    let dept_id = resolve_ulid(&mut conn, "wf_departments", &req.department_ulid).await?;

    sqlx::query(
        "INSERT INTO wf_teams (ulid, tenant_id, org_id, department_id, name, code)
         VALUES ($1, $2, $3, $4, $5, $6)",
    )
    .bind(&ulid).bind(tenant_id).bind(org_id).bind(dept_id)
    .bind(&req.name).bind(&req.code)
    .execute(&mut *conn).await?;

    emit_workforce_event(&mut conn, &auth, tenant_id, org_id, "team", &ulid,
        "team.created", serde_json::json!({"name": req.name})).await?;
...
pub async fn list_teams(
    ...
    let total: i64 = sqlx::query_scalar(
        "SELECT COUNT(*) FROM wf_teams WHERE deleted_at IS NULL",
    ).fetch_one(&mut *conn).await?;

    let items: Vec<TeamResponse> = rows.into_iter()
...
pub async fn create_position(
    ...
    let dept_id: Option<uuid::Uuid> = if let Some(ref du) = req.department_ulid {
        Some(resolve_ulid(&mut conn, "wf_departments", du).await?)
    } else {
        None
    };
    let emp_type = req.employment_type.as_ref().map(|e| e.as_str()).unwrap_or("permanent");
    ...
    .bind(&ulid).bind(tenant_id).bind(org_id).bind(dept_id)
    .bind(&req.title).bind(&req.code).bind(&req.job_grade)
    .bind(&req.description).bind(&req.responsibilities).bind(emp_type)
    .execute(&mut *conn).await?;

    emit_workforce_event(&mut conn, &auth, tenant_id, org_id, "position", &ulid,
        "position.created", serde_json::json!({"title": req.title})).await?;

    Ok((StatusCode::CREATED, Json(PositionResponse {
        ulid, title: req.title, code: req.code, job_grade: req.job_grade,
        employment_type: emp_type.to_string(), is_active: true,
        created_at: chrono::Utc::now(),
    })))
}
...
pub async fn list_positions(
    ...
    let total: i64 = sqlx::query_scalar(
        "SELECT COUNT(*) FROM wf_positions WHERE deleted_at IS NULL",
    ).fetch_one(&mut *conn).await?;

    let items: Vec<PositionResponse> = rows.into_iter()
        .map(|(ulid, title, code, job_grade, employment_type, is_active, created_at)| PositionResponse {
...
pub async fn create_employee(
    ...
    let tenant_id = resolve_tenant_id(&mut conn, &auth).await?;
    let org_id = resolve_org_id(&mut conn, &auth).await?;
    let actor_uuid = resolve_actor_uuid(&mut conn, &auth).await?;
    ...
    let dept_id: Option<uuid::Uuid> = resolve_opt(&mut conn, "wf_departments", req.department_ulid.as_deref()).await?;
    let pos_id: Option<uuid::Uuid> = resolve_opt(&mut conn, "wf_positions", req.position_ulid.as_deref()).await?;
    let loc_id: Option<uuid::Uuid> = resolve_opt(&mut conn, "wf_locations", req.location_ulid.as_deref()).await?;
    let mgr_id: Option<uuid::Uuid> = resolve_opt(&mut conn, "wf_employees", req.manager_employee_ulid.as_deref()).await?;
    ...
    .bind(&ulid).bind(tenant_id).bind(org_id)
    .bind(&req.employee_number).bind(&req.legal_name).bind(&req.preferred_name)
    .bind(&req.email).bind(&req.phone).bind(req.date_of_birth).bind(&req.gender)
    .bind(&national_id_hash).bind(&national_id_last4)
    .bind(dept_id).bind(pos_id).bind(loc_id)
    .bind(mgr_id).bind(req.hire_date).bind(actor_uuid)
    .execute(&mut *conn).await?;
    ...
    .bind(&emp_ulid).bind(tenant_id).bind(&ulid)
    .bind(pos_id).bind(dept_id).bind(loc_id).bind(emp_type)
    .bind(req.hire_date.unwrap_or_else(|| chrono::Utc::now().date_naive()))
    .bind(actor_uuid)
    .execute(&mut *conn).await?;

    // Audit: redact all sensitive fields.
    emit_workforce_event(
        &mut conn, &auth, tenant_id, org_id, "employee", &ulid,
...
pub async fn list_employees(
    ...
    let (rows, total): (Vec<EmployeeRow>, i64) = if scope_to_manager {
        ...
        let total: i64 = sqlx::query_scalar(
            "SELECT COUNT(*) FROM wf_employees WHERE deleted_at IS NULL",
        ).fetch_one(&mut *conn).await?;
    ...
    if auth.has_permission("employee.export") {
        ...
        let org_id = resolve_org_id(&mut conn, &auth).await?;
        emit_workforce_event(
            &mut conn, &auth, tenant_id, org_id, "employee", "bulk",
            "employee.exported",
...
pub async fn get_employee(
    ...
    let row = fetch_employee_row(&mut conn, &employee_ulid).await?;
    check_employee_access(&auth, &row, &mut conn).await?;
...
pub async fn update_employee_status(
    ...
    emit_workforce_event(
        &mut conn, &auth, tenant_id, org_id, "employee", &employee_ulid,
    ...
    let updated = fetch_employee_row(&mut conn, &employee_ulid).await?;
...
pub async fn update_employment(
    ...
    .bind(&emp_ulid).bind(tenant_id).bind(row.id)
    .bind(pos_id).bind(dept_id).bind(loc_id).bind(emp_type)
    .bind(req.effective_date)
    ...
    .bind(pos_id).bind(dept_id).bind(loc_id).bind(&employee_ulid)
    .execute(&mut *conn).await?;
...
pub async fn create_compensation(
    ...
    let currency_id: uuid::Uuid = sqlx::query_scalar(
    ...
    .bind(&comp_ulid).bind(tenant_id).bind(row.id)
    .bind(req.gross_amount_minor).bind(currency_id)
    .bind(req.frequency.as_str()).bind(req.effective_date)
    .bind(&req.change_reason).bind(actor_uuid)
    .execute(&mut *conn).await?;
    ...
    emit_workforce_event(
        &mut conn, &auth, tenant_id, org_id, "compensation", &employee_ulid,
        "salary.changed",
    ...
    Ok((StatusCode::CREATED, Json(CompensationResponse {
        ulid: comp_ulid,
...
pub async fn create_document(
    ...
    let newline_pos = body_bytes.iter().position(|&b| b == b'\n')
        .ok_or_else(|| AosError::Validation("body must be: JSON metadata\\n<file bytes>".to_string()))?;
    ...
    let mut conn = db.acquire().await?;
    let emp_row = fetch_employee_row(&mut conn, &employee_ulid).await?;
    check_employee_access(&auth, &emp_row, &mut conn).await?;
    let tenant_id = resolve_tenant_id(&mut conn, &auth).await?;
    let org_id = resolve_org_id(&mut conn, &auth).await?;
    let actor_uuid = resolve_actor_uuid(&mut conn, &auth).await?;
    ...
    .bind(&doc_ulid).bind(tenant_id)
    .bind(&employee_ulid)
    .bind(req.doc_type.as_str()).bind(&object_key)
    .bind(&req.filename).bind(&req.mime_type).bind(&sha256).bind(size_bytes)
    .bind(actor_uuid)
    .execute(&mut *conn).await?;

    emit_workforce_event(
        &mut conn, &auth, tenant_id, org_id, "document", &doc_ulid,
        "document.uploaded",
        serde_json::json!({
...
pub async fn list_documents(
    ...
    .bind(emp_row.id).bind(q.page_size).bind(q.offset())
    .fetch_all(&mut *conn).await?;

    let total: i64 = sqlx::query_scalar(
        "SELECT COUNT(*) FROM wf_documents WHERE employee_id = $1 AND is_active = TRUE",
    ).bind(emp_row.id).fetch_one(&mut *conn).await?;
...
pub async fn get_document_url(
    ...
    let (presigned_url, ttl) = presigned_url_for_doc(&state.s3_client, &state.s3_cfg, &doc_row).await?;
    ...
    Ok(Json(DocumentUrlResponse { presigned_url, expires_in_secs: ttl }))
}
...
async fn check_employee_access(
    ...
    Err(AosError::Forbidden("not authorized to view this employee record".to_string()))
}
...
fn employee_row_to_response(row: EmployeeRow) -> EmployeeResponse {
    // ULID lookups for FK ULIDs would require extra queries — omitted for MVP.
    // ponytail: JOIN against wf_departments/positions/locations for ULID in
    // the SELECT if the frontend needs them in the list view.
    EmployeeResponse {
        ulid: row.ulid,
        ...
        current_department_ulid: row.current_department_id.map(|u| u.to_string()),
        current_position_ulid: row.current_position_id.map(|u| u.to_string()),
        current_location_ulid: row.current_location_id.map(|u| u.to_string()),
        manager_employee_ulid: row.manager_employee_id.map(|u| u.to_string()),
